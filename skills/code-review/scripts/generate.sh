...
PHASES=(init setup agents synthesize report route)
...
build_json() {
    # Usage: build_json key1 value1 key2 value2 ...
    # All values are passed as --arg or --argjson to jq -n
    local jq_args=()
    while [[ $# -ge 2 ]]; do
        local key="$1"
        local val="$2"
        shift 2
        # Check if value is valid JSON (starts with { or [ or is a number/bool/null)
...
if [[ -z "$PHASE" ]]; then
    echo '{"error": "Missing --phase argument", "details": "Use --phase with one of: init, setup, agents, synthesize, report, route"}'
    exit 1
fi
...
4. If the codebase exceeds size thresholds, chunk files into batches of ~2500 lines. Every agent must review every chunk — do NOT assign different file subsets to different agents.
5. If the codebase exceeds 10000 lines, apply sampling per the diff_acquisition instructions. Every agent reviews the same sampled files.
6. After acquiring the file list, call generate.sh with --phase agents, passing the same JSON payload.
...
You are reviewing complete source files, not a diff. Report issues found anywhere in the provided files. Focus on the most impactful problems — do not exhaustively list minor style issues across the entire codebase. Prioritize correctness and security over style in a full-review context."
        ...
        DISPATCH_PROMPT="You are the dispatch orchestration agent for a full codebase review. You have the agent prompts and the source files. For each agent in agent_prompts:

1. Load the agent prompt (it includes experimental-model and full-review framing context).
2. Load the source file contents via the Read tool (not Bash). Every agent must receive ALL source files (or all chunks if chunked). Do NOT assign different file subsets to different agents.
...
2. Load all individual agent results from /tmp/codereview_<agent>_result.json.
